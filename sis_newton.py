...
def newton(S2_e,S1,P2_e,qp_e,X2_e,Xw_e,mium,Kis,Ks,b,g,Kp,Kip,Pm_sub,Pm_prod,qm,Fw,Fs,V,qs,Fr,F,miu,e1,e2,divergente):
    ...
    c = 0
    while (e1 > 10 or e2 > 10) and c<=500: #enquanto o erro não for aceitável
        c += 1
        ...
        #atualização do variáveis
        FUN_n = np.array([[-f1(miu, mium, S2, Ks, Kis, P2, Pm_sub, b)],
                        [-f2(qp, qm, S2, Kp, Kip, P2, Pm_prod, g)],
                        [-f3(F, S1, qs, X2, V, Fs, Fw, S2)],
                        [-f4(qp, X2, V, Fw, P2, Fs)],
                        [-f5(F, Fr, X2, Fw, Xw)]])

        e1 = np.linalg.norm(FUN_n)
