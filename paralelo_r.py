...
def show_results(plot=False):
    if plot: 
        ...
        for i in x_Fw_gluc_p:
            axs[1].axvline(x=i, color = 'b', **{'linestyle': 'dashed'})
    ...
    print('\nREATORES ABERTOS EM PARALELO (com reciclagem e purga)')
    print('Valores fixos:')
    print('V = ', V)
    ...
    for Fw in Fw_r: # Fw varia na gama de valores definida
        ...
        for Fr in Fr_r: # Fr varia na gama de valores definida
            ...
            if P_out > P_out_max and Fr>=0 and S2>=0 and Fs>=0 and X2<X2_max_c and X2>0:
                ...
                P_out_max = P_out
                X2_max = X2
                if plot: #atualização de variáveis para gráficos
                    P_out_max_p.append(P_out_max)
                    x_max.append(c)
